...
var FIELDNAME_MEDIAURL = "media_URL"; //modified for another media different from logo - Mingda
...
function buildPopup(feature, geometry, baseLayerClick)
{
	...
	var picture = atts.getValueCI(FIELDNAME_MEDIAURL); //modifed for another Media different from logo - Mingda
	...
	if (picture) {
		...
		if (DETAILS_PANEL && !mobile) {
			...
			if (website) {
				...
				var mobileA = $("<a></a>").attr("href", website).attr("target","_blank");
				if (picture.match(".youtube.")!=null){ //detect video or picture - Mingda
					$(pDiv).append("<iframe id='youtubevideo' width=90% height='200'  src='"+picture+"'frameborder='0' allowfullscreen></iframe>");
					$(mobilePDiv).append("<iframe id='youtubevideo' width=90% height='200'  src='"+picture+"'frameborder='0' allowfullscreen></iframe>");
				} else {
				...
				if (picture.match(".youtube.")!=null){ //detect video or picture - Mingda
					$(pDiv).append("<iframe width='203' height='200'  src='"+picture+"'frameborder='0' allowfullscreen></iframe>");
					$(mobilePDiv).append("<iframe width='203' height='200'  src='"+picture+"'frameborder='0' allowfullscreen></iframe>");
				} else {
				$(pDiv).append($(new Image()).attr("src", picture));
				$(mobilePDiv).append($(new Image()).attr("src", picture));
				}
	...
	$.each(features, function(index, feature){
		...
		if (picture) {
			...
			if (website) {
				...
				if (picture.match(".youtube.")!=null){ //detect video or picture - Mingda
					$(mobilePDiv).append("<iframe width=90% height='200'  src='"+picture+"'frameborder='0' allowfullscreen></iframe>");
				}else{
				$(mobilePDiv).append($(new Image()).attr("src", picture));
				}
			}
